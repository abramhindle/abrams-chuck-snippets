...
while( true ) {
    ...
    //(t*13 + x >> 3) => x; // 1 tone
    //t * (t*13 + x >> 3) => x; // noise
    (t16*(99^t&t>>12) + t*t*t*t>>3) + ((t>>3)*((t>>3)|(t&t>>9))^(63&(t&t>>12)|(t*t>>4)|(t>>5)))=> x; //squishy high
    
    //t | ((t>>3)*((t>>3)|(t&t>>9))^(63&(t&t>>12)|(t*t>>4)|(t>>5)))=> x; //squishy high
    //(x<<5 + y<<3)%45 >> 5 => y;
    ...
    //(t*t*t & 23) | (-1 * t<<4) |  ((1-t)>>9) | (t>>6) | (t*t >> 5) => x;
...
//(3*(t-t/7)/99)*(
    ...
    //t*(((t>>16)|(22^t>>17)|(t20>>7)^(t*t>>9))^(244&(t>>7)|(t16>>5))) => x; //with beat
